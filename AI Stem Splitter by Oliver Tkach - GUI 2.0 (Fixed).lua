...
local function build_demucs_command_for_runner(file_path, work_dir)
    ...
    -- FIX: Force backslashes for Windows paths to avoid mixed separator issues
    if IS_WINDOWS then
        file_path = file_path:gsub("/", "\\")
        work_dir = work_dir:gsub("/", "\\")
        filename_template = filename_template:gsub("/", "\\")
        
        -- FIX: Robust quoting structure for cmd.exe
        -- We wrap the python command in quotes, and arguments in quotes
        return table.concat({
    ...
    -- Keep Mac/Linux logic as is (or standard)
    return table.concat({
        "$PY_CMD -m demucs.separate",
        " -n ", MODEL_NAME,
        " --filename ", quote_arg(filename_template),
...
local function build_windows_runner_async(work_dir, song_name, file_path, log_path, status_path)
    -- Normalize paths for the batch file variables
    local safe_file_path = file_path:gsub("/", "\\")
    local safe_work_dir = work_dir:gsub("/", "\\")
    local demucs_cmd = build_demucs_command_for_runner(safe_file_path, safe_work_dir)
    
    local probe_path = path_join(work_dir, "torchaudio_write_probe.wav"):gsub("/", "\\")
    local analysis_script_path = path_join(work_dir, "analyze_audio.py"):gsub("/", "\\")
    local analysis_output_path = path_join(work_dir, "analysis_info.txt"):gsub("/", "\\")

    -- Generate Analysis Script (remains the same)
    local analysis_script_content = build_analysis_script(safe_file_path, analysis_output_path, safe_work_dir)
    ...
    return table.concat({
        ...
        "",
        ":run",
        "echo [preflight] Detecting Python...",
        ...
        ")",
        "",
        "echo [preflight] Probing WAV export...",
        "\"!PY_CMD!\" -c \"import os, torch, torchaudio as ta; p='" .. escape_python_single_quoted(probe_path) .. "'; x=torch.zeros(2, 512); ta.save(p, x, 44100); os.remove(p)\" > nul 2>&1",
        "if errorlevel 1 (",
        "  set \"EXTRA_ARGS=--mp3 --mp3-bitrate 320\"",
        "  echo [preflight] Fallback to MP3.",
        ")",
        "",
