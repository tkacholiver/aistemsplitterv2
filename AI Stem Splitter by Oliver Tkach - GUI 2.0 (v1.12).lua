--[[
    ...
    Role: Professional Audio Separation Engine (Universal V3 - Auto-Install)
    Target: Windows & Linux (Mac logic removed for stability)
    Features: 
...
local APP_NAME = "AI Stem Splitter by Oliver Tkach - Universal 3.0"
...
local function build_setup_help(python_cmd)
    if IS_WINDOWS then
        ...
        return table.concat({
            ...
            "2) pip3 install demucs soundfile librosa numpy scipy",
        }, "\n")
    end
end
...
local function build_linux_runner_async(work_dir, song_name, file_path, log_path, status_path)
    ...
    return table.concat({
        ...
        "    \"$PY_CMD\" -m pip install demucs soundfile librosa numpy scipy >/dev/null 2>&1",
        "    if [ $? -ne 0 ]; then",
        "      echo \"[error] Auto-install failed.\"",
        "      CODE=12",
        "      return",
